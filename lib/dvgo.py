...
class DirectVoxGO(torch.nn.Module):
    ...
    def scale_volume_grid(self, num_voxels):
        ...
        if self.mask_cache_path is not None and self.mask_cache_path:
            mask_cache = MaskCache(
            ...
            mask = mask_cache(self_grid_xyz) & (self_alpha>self.fast_color_thres)
        else:
            mask = torch.ones(list(self.world_size), dtype=torch.bool)
        self.mask_cache = MaskCache(
                path=None, mask=mask,
    ...
    def render_rays(self, N, ray_pts, ray_id, step_id, viewdirs, global_step=None, **render_kwargs):
        '''Volume rendering'''
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        return self.render_rays(N, ray_pts, ray_id, step_id, viewdirs, global_step, **render_kwargs)
...
''' Ray and batch
'''
def get_rays(H, W, K, c2w, inverse_y, flip_x, flip_y, mode='center', ray_forward=-1):
    i, j = torch.meshgrid(
    ...
    if inverse_y:
        dirs = torch.stack([(i-K[0][2])/K[0][0], (j-K[1][2])/K[1][1], ray_forward * -torch.ones_like(i)], -1)
    else:
        dirs = torch.stack([(i-K[0][2])/K[0][0], -(j-K[1][2])/K[1][1], ray_forward * torch.ones_like(i)], -1)
...
def get_rays_nhr(H, W, K, c2w):
    M = 1

    K = torch.tensor(K, device=c2w.device, dtype=torch.float)
    ...
    dirs = torch.cat([dirs, torch.zeros(dirs.size(0), coordinates.size(1), coordinates.size(2), 1, 1, device=K.device)],dim=3) # (M,H,W,4,1)


    dirs = torch.matmul(c2w, dirs) # (M, H, W, 4, 1)
    dirs = dirs[:, :, :, 0:3, 0] # (M, H, W, 3)

    rays_d = dirs.squeeze(0)
    rays_o = c2w[:3, 3].expand(rays_d.shape)
    return rays_o, rays_d
...
def get_rays_of_a_view(H, W, K, c2w, ndc, inverse_y, flip_x, flip_y, mode='center', camera_type=None):
    if camera_type == 'nhr':
        rays_o, rays_d = get_rays_nhr(H, W, K, c2w)
    else:
