...
__global__ void sample_pts_on_rays_cuda_kernel(
  ...
  if(idx<total_len) {
    ...
    // float rand_noise_dist = curand_uniform(&(rand_states[threadIdx.x])) + 1e-6f - 0.5f;
...
template <typename scalar_t>
__global__ void infer_rays_hit_bbox_cuda_kernel(
        scalar_t* __restrict__ t_min,
        scalar_t* __restrict__ t_max,
        const float eps,
        const int n_rays,
        bool* __restrict__ is_hit) {
  const int i_ray = blockIdx.x * blockDim.x + threadIdx.x;
  if(i_ray < n_rays) {
    is_hit[i_ray] = abs(t_max[i_ray]-t_min[i_ray]) > eps;
  }
}
...
std::vector<torch::Tensor> sample_pts_on_rays_cuda(
  ...
  // setup_random_kernel<<<1, 1>>>(d_states);
...
std::vector<torch::Tensor> sample_pts_on_rays_depth_cuda(
        torch::Tensor rays_o, torch::Tensor rays_d, torch::Tensor depth,
        torch::Tensor xyz_min, torch::Tensor xyz_max,
        const float depth_sampling_std, const float near, const float far, const float stepdist) {
  const int threads = 256;
  const int n_rays = rays_o.size(0);
  ...
  // Create a depth-cueing sample interval
  auto rnorm = torch::norm(rays_d, 2, 1);
  auto intvl_width = torch::abs(torch::randn({n_rays}, depth.options()) * depth_sampling_std);
  auto t_min = torch::maximum(t_minmax[0], (depth - intvl_width) / rnorm);
  auto t_max = torch::minimum(t_minmax[1], (depth + intvl_width) / rnorm);
  ...
  return {rays_pts, mask_outbbox, ray_id, step_id, N_steps, t_min * rnorm, t_max * rnorm};
}
...
torch::Tensor rays_hit_test_bbox_cuda(
        torch::Tensor rays_o, torch::Tensor rays_d,
        torch::Tensor xyz_min, torch::Tensor xyz_max,
        const float near, const float far, const float stepdist) {
  const int threads = 256;

  // Compute ray-bbox entry and exit distance
  auto t_minmax = infer_t_minmax_cuda(rays_o, rays_d, xyz_min, xyz_max, near, far);
  auto t_min = t_minmax[0];
  auto t_max = t_minmax[1];

  // Find ray-bbox intersection
  const int n_rays = t_min.size(0);
  const int blocks = (n_rays + threads - 1) / threads;

  auto is_hit_bbox = torch::empty({n_rays}, torch::dtype(torch::kBool).device(torch::kCUDA));

  AT_DISPATCH_FLOATING_TYPES(t_min.type(), "rays_hit_test_bbox_cuda", ([&] {
    infer_rays_hit_bbox_cuda_kernel<scalar_t><<<blocks, threads>>>(
        t_min.data<scalar_t>(),
        t_max.data<scalar_t>(),
        stepdist / 2,
        n_rays,
        is_hit_bbox.data<bool>());
  }));
