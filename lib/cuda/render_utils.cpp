...
std::vector<torch::Tensor> sample_pts_depth_cueing_on_rays(
        torch::Tensor rays_o, torch::Tensor rays_d, torch::Tensor depth,
        torch::Tensor xyz_min, torch::Tensor xyz_max,
        const float depth_sampling_std, const float near, const float far, const float stepdist) {
  CHECK_INPUT(rays_o);
  ...
  assert(rays_o.dim()==2);
  assert(rays_o.size(1)==3);
  return sample_pts_on_rays_depth_cuda(rays_o, rays_d, depth, xyz_min, xyz_max, depth_sampling_std, near, far, stepdist);
}
...
torch::Tensor rays_hit_test_bbox(
        torch::Tensor rays_o, torch::Tensor rays_d,
        torch::Tensor xyz_min, torch::Tensor xyz_max,
        const float near, const float far, const float stepdist) {
  CHECK_INPUT(rays_o);
  ...
  assert(rays_o.dim()==2);
  assert(rays_o.size(1)==3);
  return rays_hit_test_bbox_cuda(rays_o, rays_d, xyz_min, xyz_max, near, far, stepdist);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("sample_pts_depth_cueing_on_rays", &sample_pts_depth_cueing_on_rays, "Depth-cueing point sampling on rays");
  m.def("rays_hit_test_bbox", &rays_hit_test_bbox, "Ray-bbox hit testing");
